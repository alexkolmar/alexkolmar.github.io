class TagBuilder {
 ...
 initEventListeners() {
  ...
  // КАСТОМНЫЕ ПОЛЯ
  this.setupCustomField('customAge', 'age');
  ...
  this.setupCustomField('customFacialHair', 'facialHair');
  ...
  this.setupCustomField('customMakeup', 'makeup');
  this.setupCustomField('customBodyMods', 'bodyMods');
 ...
 updateOutput() {
  ...
  // ВОЗРАСТ
  const combinedAge = this.getCombinedTags('age');
  if (combinedAge.length > 0) {
   finalTags.push(combinedAge.join(', '));
  }
  ...
  // Элементы макияжа + кастомное (все через getCombinedTags)
  const combinedMakeup = this.getCombinedTags('makeup');
  if (combinedMakeup.length > 0) {
   makeupParts.push(...combinedMakeup);
  }

  // Стиль макияжа (отдельная категория)
  if (this.tags.makeupStyle.length > 0) {
   makeupParts.push(...this.tags.makeupStyle);
  }

  // Собираем тег макияжа
  if (makeupParts.length > 0) {
   finalTags.push(makeupParts.join(' '));
  }

  // НОВЫЙ БЛОК: ТЕЛЕСНЫЕ МОДИФИКАЦИИ
  // Используем только getCombinedTags, чтобы избежать дублирования
  const combinedBodyMods = this.getCombinedTags('bodyMods');
  if (combinedBodyMods.length > 0) {
   finalTags.push(combinedBodyMods.join(', '));
  }
